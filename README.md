...
- 当前源码已通过 141 项自动化测试、类型检查和生产构建；本地完整安装包、轻量更新包与打包后健康检查也已通过。GitHub Hosted Windows 手动预检 Run `33431193467` 已验证 v0.9.31 → v0.9.39 成功更新、100% 后关窗不自动重启、数据保留，以及可见/静默失败回滚；该预检未发布 Release。Stable Cloud 托管入口尚需改为桌面客户端可直接访问，并配置真实供应商模型与密钥后，才会发布 GitHub Release。
