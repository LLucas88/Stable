...
test('store persists resources, retrieval, workflow and messages', () => {
  ...
  try {
    ...
    assert.equal(store.searchConversations('已开始')[0].id, firstConversationId)
    assert.equal(store.searchConversations('分析 第二份')[0].id, secondConversationId)
    assert.equal(store.searchConversations('不存在的关键词').length, 0)
    assert.equal(store.searchConversations('').length, 2)
    assert.equal(store.conversation(secondConversationId).pinned, false)
    store.setConversationPinned(secondConversationId, true)
    assert.equal(store.conversation(secondConversationId).pinned, true)
    ...
    assert.equal(reopened.listConversations().find((item) => item.title === '分析第二份任务').pinned, true)
    reopened.close()
