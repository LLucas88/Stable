...
const { BuiltinTools } = require(resourcesPath ? path.join(resourcesPath, 'app.asar/desktop/services/builtin-tools.cjs') : '../desktop/services/builtin-tools.cjs')
...
async function main() {
  ...
  let steerSeen; let releaseSteer; let steerGate
  const server = http.createServer(async (req, res) => {
    let raw = ''; for await (const chunk of req) raw += chunk
    const body = JSON.parse(raw); requests.push(body)
    if (req.url !== '/v1/chat/completions') { res.writeHead(404); res.end(); return }
    if (phase === 'cancel') { cancelSeen(); return }
    if (phase === 'steer' && !issuedTool) { issuedTool = true; steerSeen(); await steerGate }
    ...
    if (phase.startsWith('builtin-') && !issuedTool) {
      issuedTool = true
      const tool = body.tools.find((item) => item.function.name.endsWith('stable_excel'))
      assert.ok(tool, 'Native Codex must expose the registered Excel tool')
      const args = phase === 'builtin-create'
        ? { action: 'create', output: 'native.xlsx', sheets: [{ name: 'Sales', rows: [['店', '金额'], ['A店', 170]] }] }
        : { action: 'read', path: 'native.xlsx', sheet: 'Sales' }
      expectedThoughts.set(`call_${phase}`, thought)
      chunks = [
        { choices: [{ index: 0, delta: { reasoning_content: thought, tool_calls: [{ index: 0, id: `call_${phase}`, type: 'function', function: { name: tool.function.name, arguments: JSON.stringify(args) } }] }, finish_reason: null }] },
  ...
  try {
    ...
    const builtinOptions = { ...options, builtinTools: () => new BuiltinTools({ workspace, electron: {}, packaged: Boolean(resourcesPath), resourcesPath }) }
    for (const builtinPhase of ['builtin-create', 'builtin-read']) {
      phase = builtinPhase; issuedTool = false
      await new CodexHarnessRunner(builtinOptions).run(builtinPhase, model, apiKey, 45000, publish, 'workspace-write', [], { key: 'builtin' })
      assert.ok(fs.existsSync(path.join(workspace, 'native.xlsx')))
      const toolResults = requests.at(-1).messages.filter((message) => message.role === 'tool')
      assert.match(JSON.stringify(toolResults), builtinPhase === 'builtin-read' ? /A店/ : /native.xlsx/)
    }
    phase = 'steer'; issuedTool = false
    const steerRequested = new Promise((resolve) => { steerSeen = resolve })
    steerGate = new Promise((resolve) => { releaseSteer = resolve })
    currentRunner = new CodexHarnessRunner(options)
    const steeringRun = currentRunner.run('INITIAL_DIRECTION', model, apiKey, 45000, publish, 'read-only')
    await Promise.race([steerRequested, steeringRun.then(() => { throw new Error('Turn completed before steering') })])
    try { await currentRunner.steer('UPDATED_DIRECTION') } finally { releaseSteer() }
    await steeringRun
    assert.equal(currentRunner.steerReady, false)
    assert.match(JSON.stringify(requests.at(-1).messages), /UPDATED_DIRECTION/)
