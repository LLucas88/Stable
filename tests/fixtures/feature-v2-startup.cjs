...
const {app,BrowserWindow}=electron,repo=path.resolve(__dirname,'../..'),root=fs.mkdtempSync(path.join(repo,'qa-artifacts/feature-v2/startup-'))
...
let errors=[];process.on('unhandledRejection',error=>errors.push(String(error)))
