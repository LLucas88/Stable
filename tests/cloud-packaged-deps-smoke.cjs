...
app.whenReady().then(() => {
  const asar = path.resolve(process.env.STABLE_TEST_ASAR || path.join(__dirname, '..', pkg.build.directories.output, 'win-unpacked/resources/app.asar'))
  assert.ok(fs.existsSync(asar))
  const packagedRequire = createRequire(path.join(asar, 'package.json'))
  assert.equal(packagedRequire('./package.json').version, pkg.version)
  ...
  const result = { version: pkg.version, dependencies: 7, services: services.length, sourceHashesMatch: true }
