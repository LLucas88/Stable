...
test('desktop adapter delivers in-app fallback and notification click restores the matching task',()=>{
 ...
 const alerts=vm.runInNewContext(block+';taskAlerts',{require:name=>require('../desktop/services/task-alerts.cjs'),Notification,mainWindow,store,agentState:id=>({activeConversationId:id}),setInterval:()=>({unref(){}})})
