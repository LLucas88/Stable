...
test('production styles preserve material layering and light/dark solid fallbacks',{timeout:40000},async()=>{
 const env={...process.env};delete env.ELECTRON_RUN_AS_NODE
 const result=await new Promise(resolve=>{
  const child=spawn(require('electron'),[path.join(__dirname,'fixtures/codex-surfaces-ui.cjs')],{cwd:path.join(__dirname,'..'),env,windowsHide:true,stdio:['ignore','pipe','pipe']})
  let output=''
 ...
 assert.equal(result.code,0,result.output);assert.match(result.output,/CODEX_SURFACES_UI_PASSED/)
})
