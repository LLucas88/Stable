...
test('authenticated cloud catalog, selection and bridge retain official effort across local gateway',async()=>{
 const requests=[]
 const account={baseURL:'https://cloud.test',token:()=> 'fixture',publicState:()=>({status:'authenticated',models:[{id:'glm-5.3-flash',provider:'zhipu',display_name:'GLM'}]})}
 ...
 requests.forEach(x=>{assert.deepEqual(x.messages,requests[0].messages);assert.equal(x.thinking,undefined)})
