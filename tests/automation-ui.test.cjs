...
test('GitHub Releases updater is configured for the Stable repository', () => {
  ...
  assert.equal(pkg.build.directories.output, 'release-0.91.7')
  ...
  assert.match(workflow, /-UpdateInstaller "\.\/release-\$version-update\/Stable-Update-\$version-x64\.exe"/)
  assert.match(workflow, /-ExpectedVersion \$version/)
