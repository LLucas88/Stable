...
class NaposAPIBase:
    ...
    def _load_config_if_needed(self, config_module, require_auth: bool = True):
        ...
        if require_auth and not self.get_token():
            import os
            if os.environ.get('WENDING_CONFIG_DIR'):
                raise RuntimeError('此任务尚未登录问鼎，请打开任务顶部的问鼎 CLI 绑定账号和品牌。')
            raise RuntimeError(
                "未登录或登录已过期，请先运行:\n"
    ...
    def _do_request(self, url, body, headers, timeout=3, method="POST", use_gateway=True):
        from .stable_scope import request_lock
        with request_lock(self):
            return self._do_scoped_request(url, body, headers, timeout, method, use_gateway)

    def _do_scoped_request(self,
                    url: str,
                    body: dict,
                    headers: dict,
                    timeout: int = 3,
        ...
        from .stable_scope import before_request, verify
        scoped = before_request(self, headers, use_gateway)
        ...
        try:
            with urllib.request.urlopen(req, timeout=timeout) as resp:
                ...
                value = self._handle_response(result)
                if scoped:
                    verify(self)
                return value
