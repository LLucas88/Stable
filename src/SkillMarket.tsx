...
export function SkillMarket({ onUse, renderContent }: { onUse: (state: AgentState) => void; renderContent: (content: string) => ReactNode }) {
  ...
  return <section className="skill-market" aria-label="技能市场">
    <header className="skill-market-top"><nav aria-label="市场类型">{([['skill','Skill技能'],['connector','cli&mcp连接器'],['expert','Agent专家']] as const).map(([id,label]) => <button key={id} role="tab" aria-selected={tab === id} onClick={() => { setTab(id); setGroup('全部'); setNotice('') }}>{label}</button>)}</nav>
    ...
    {available.some(item => item.bundled) && <p className="market-feedback" role="status">已入库 {available.length} 个技能版本 · 启用 {available.filter(item => item.enabled).length} 个 · 飞书相关技能暂不启用</p>}
