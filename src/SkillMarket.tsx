...
export function SkillMarket({ onUse, renderContent }: { onUse: (state: AgentState) => void; renderContent: (content: string) => ReactNode }) {
  ...
  const visible = presentedItems.filter(item => item.kind === tab && (mine ? item.installed : item.builtin || item.bundled) && (group === '全部' || item.group === group) && `${item.name} ${item.description} ${(item.tags || []).join(' ')} ${item.source || ''}`.toLowerCase().includes(query.trim().toLowerCase()))
