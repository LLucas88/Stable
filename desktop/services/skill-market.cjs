...
class SkillMarket {
  ...
  toggle(id,enabled) { if(id===WENDING.id)this.metadata(id,{enabled,installed:true});else this.store.setSkillEnabled(id,enabled);return this.entries() }
  remove(id) { if(id===WENDING.id)this.metadata(id,{installed:false,enabled:false});else {this.store.removeSkill(id);this.metadata(id,{removed:true})}return this.entries() }
