...
function wendingCliAgentInstruction() {
  return [
    ...
    '- 禁止猜测或批量枚举数据集 ID。仅使用文档或命令明确提供的数据集；没有目标指标的可靠入口时直接说明缺口。',
    '- session.lock 的 Permission denied 是本地环境错误，不要循环重试、修改锁目录或删除锁文件；停止查询并报告路径。',
    '- 同一帮助命令只读一次；不要为实时查询递归扫描磁盘寻找历史数据或逆向 CLI。业务 success=false 即失败，即使进程退出码为 0。',
