...
After adding or updating a skill, keep these **10 tracking surfaces** in sync. **This list is authoritative** — `CLAUDE.md` and `AGENTS.upstream.md` point here instead of restating it, so update this list if the set changes.
...
- `CLAUDE.md`, `AGENTS.upstream.md`, and `SECURITY.md` — authored policy plus generated current-release marker regions
