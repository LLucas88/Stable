...
技能 ID：`aaron-cold-outbound-sequencer`

原始名称：cold-outbound-sequencer

中文用途概括：Use when the user asks to "build a B2B cold-outbound sequence", "design reply-triage branching", "plan a domain warmup / sending throttle", or "make my outbound CAN-SPAM / opt-in compliant"; produces a multi-step outbound sequence with reply-triage branches (positive / objection / referral / not-now / opt-out), a warmup + send-throttle ramp schedule, jurisdiction opt-in/CAN-SPAM guardrails (guidance, not legal advice), and a SEND S-dimension read

适用方向：运营岗位、会员运营。四方向评分：{"internet": 0, "operations": 65, "membership": 53, "content": 0}。评分是可复现规则辅助判断，未在你的业务环境测得精确相关度；证据见根目录 manifest.json。
...
来源：https://github.com/aaron-he-zhu/aaron-marketing-skills/tree/c7d03b11e0a4ec837506db21fc62de70bbcfb807/email/deliver/cold-outbound-sequencer
