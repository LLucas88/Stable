---
name: aaron-cold-outbound-sequencer
...
metadata:
  source-name: cold-outbound-sequencer
  bundle: ops-expanded-2026-09-06
  upstream-slug: aaron-cold-outbound-sequencer
  upstream-displayName: Cold Outbound Sequencer · B2B冷启动外联序列
  upstream-summary: B2B冷启动外联序列/回复分流/域名预热
  upstream-version: 20.1.0
  upstream-homepage: https://github.com/aaron-he-zhu/aaron-marketing-skills
  upstream-when_to_use: 'Use when designing a B2B cold-outbound email program before writing the individual emails: a multi-step prospecting sequence with per-step timing and exit rules, the reply-triage branching that routes each reply type, a domain/mailbox warmup ramp and per-mailbox sending throttle to protect deliverability, and the CAN-SPAM / opt-in jurisdiction guardrails the sequence must respect. Activate when the user has a target list or ICP and wants the sequence map, the warmup/throttle schedule, and the compliance guardrails before creative or send-testing begins. Not for consented B2C lifecycle automation and not for adjudicating the consent record itself.'
  upstream-argument-hint: <sequence goal or ICP> [sending domain/mailbox setup] [target jurisdiction(s)] [list source]
  upstream-metadata: '{"author": "aaron-he-zhu", "version": "20.1.0", "discipline": "email", "phase": "deliver", "geo-relevance": "low", "hermes": {"tags": ["marketing", "email", "deliver"], "category": "email"}, "openclaw": {"emoji": "✉️", "homepage": "https://github.com/aaron-he-zhu/aaron-marketing-skills"}}'
license: Apache-2.0
