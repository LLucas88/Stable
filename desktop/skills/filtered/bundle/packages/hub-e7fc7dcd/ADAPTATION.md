...
技能 ID：`hub-e7fc7dcd-wagtail-django-cms`

原始名称：Wagtail Django CMS 用于结构化内容和编辑工作流

中文用途概括：Wagtail 是一个基于 Django 构建的开源 CMS，专为需要结构化内容、灵活页面模型和专业编辑体验的团队而设计。它适用于传统网站和无头构建，内置内容 API、图像处理、搜索和多站点支持功能。

适用方向：互联网业务、内容运营。四方向评分：{"internet": 50, "operations": 0, "membership": 0, "content": 92}。评分是可复现规则辅助判断，未在你的业务环境测得精确相关度；证据见根目录 manifest.json。
...
来源：https://skillhub.cn/skills/wagtail-django-cms-structured-content-editorial-workflows?namespace=user_3c6cb52e
