...
contextBridge.exposeInMainWorld('stable', {
  ...
  agent: {
    ...
    messages: (id, beforeCursor) => invoke('stable:agent:messages', { id, beforeCursor }),
    viewState: (id, value) => invoke('stable:agent:viewState', { id, value }),
    ...
    catchReply: (conversationId, messageId) => invoke('stable:agent:catch', { conversationId, messageId }),
    discardCatch: (conversationId) => invoke('stable:agent:discardCatch', { conversationId }),
    state: (id) => invoke('stable:agent:state', { id }),
    search: (query, offset = 0) => invoke('stable:agent:search', { query, offset }),
    ...
    openWorkspace: (id) => invoke('stable:agent:openWorkspace',{id}),
    ...
    clarificationTimer: (conversationId, id, action) => invoke('stable:agent:clarification-timer', { conversationId, id, action }),
    run: (conversationId, prompt, attachments = [], references = [], clientRequestId, clarificationResponse) => invoke('stable:agent:run', { conversationId, prompt, attachments, references, clientRequestId, clarificationResponse }),
