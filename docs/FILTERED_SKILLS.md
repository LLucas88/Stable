...
允许的入口：技能市场“在对话中试用”、聊天手动引用技能、以“调用/使用 + 技能完整名称或登记 ID”开头的明确指令，以及用户手动运行已经配置技能节点的工作流。普通任务不匹配技能；定时触发、团队自动分配和 AI 按相关性规划都不能自动选择技能。AI 工作流生成只获得用户在目标中明确点名的技能。

执行层同时关闭旧的 `retrieveSkills`、`enabledSkillContent` 自动正文入口；`searchSkills` 只保留目录搜索。主对话、自动任务和团队路径区分人工调用上下文，Codex Harness 每轮也收到手动调用规则，历史调用不构成本轮授权。原有禁用和删除选择保留。
